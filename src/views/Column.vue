    ...
    <div id='coldata'>
      ...
      <div id='affpapers' class="datawrapper">
        ...
        <div
            ...
            style="width: 80px;height:30px;align-items:center;
            background-color:#26BEB8;color: white;border-radius: 10px;
            margin-right:1px;"
  ...
  methods: {
    nowcol(pub){
      ...
      for(var i=0;i<this.column.length;i++){
        if(this.column[i]==pub){console.log(this.columnnum[i]);return this.columnnum[i];}
      }
      return -1;
    },
    papersincol(col){
      ...
      var _this = this
        axios.post('https://go-service-296709.df.r.appspot.com/api/v1/portal/column/list_all_from_column',formData, config)
    ...
    mouseLeaveWrapper() {
      ...
      var _this = this
        axios.post('https://go-service-296709.df.r.appspot.com/api/v1/portal/column/searchcol',formData, config)
            .then(function (response)  {
                ...
                if(response){
                  if(response.data.success){
                    ...
                    for(var i=0;i<response.data.data.length;i++){
                      if(response.data.data[i].column_name  in _this.column)
                        continue;
